...
async function releaseSingle(
  ...
  if (!dry) {
    const confirmed = await prompt.confirm("Continue with release?", {
      default: true,
    });
    if (!confirmed) {
  ...
  const steps = [
    ...
    { name: "Updating version", cmd: () => updatePackageVersion(newVersion) },
    {
      ...
      cmd: () => createGitTag(newVersion, config, flags),
    },
    { name: "Publishing to npm", cmd: () => publishToNpm(flags, config) },
    {
      name: "Creating GitHub release",
      cmd: () => createGitHubRelease(newVersion, flags, config),
    },
  ];

  for (const step of steps) {
    if (step.name.includes("npm") && !(flags.npm ?? config.release?.npm ?? true)) {
      continue;
    }
    if (step.name.includes("GitHub") && !(flags.github ?? config.release?.github ?? true)) {
      continue;
    }
  ...
  if (flags.github ?? config.release?.github ?? true) {
    console.log(
      relico.dim(`GitHub: https://github.com/${await getGitHubRepo()}/releases/tag/v${newVersion}`)
    );
  }
...
async function createGitHubRelease(version: string, flags: any, config: any) {
  ...
  const tag = `v${version}`;
  const _repo = await getGitHubRepo();
