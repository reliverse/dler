...
export default defineCommand({
  ...
  handler: async ({ flags, positional, spinner }) => {
        ...
        const watchCommands = async () => {
          try {
            ...
            for await (const event of watcher) {
              ...
              if (!event.filename?.match(/\.(ts|tsx|js|jsx)$/)) {
                continue;
              }

              // Skip generated files
              if (event.filename?.includes("commands.gen.ts")) {
                continue;
              }
              if (event.filename?.includes(".dler/")) {
                continue;
              }
