// apps/dler/src/cmds/build/plugins/typescript-declarations.ts
...
export const TypeScriptDeclarationsPlugin: DlerPlugin = {
  ...
  onBuildEnd: async (result: BuildResult) => {
    ...
    try {
      await generateTypeDeclarations(pkg, result);
    } catch (error) {
...
async function generateTypeDeclarations(pkg: PackageInfo, result: BuildResult): Promise<void> {
