...
export default defineCommand({
  ...
  handler: async ({ flags, positional }) => {
    ...
    // Build @reliverse/dler rempts command
    const args = ["rempts"];
    ...
    // Run @reliverse/dler rempts via bunx
    ...
    proc.on("exit", (code) => {
      if (code === 0) {
        ...
        console.log(relico.gray("  @reliverse/dler rempts dev"));
      } else {
    ...
    proc.on("error", (error) => {
      ...
      console.log();
      console.log("Make sure @reliverse/dler rempts is available:");
      console.log(relico.gray("  bunx @reliverse/dler rempts --help"));
      process.exit(1);
    });
  },
});
