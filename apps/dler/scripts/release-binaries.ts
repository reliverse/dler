...
console.log(`🚀 Building @reliverse/dler rempts ${version} for all platforms...\n`);
...
for (const platform of platforms) {
  ...
  const filename = `rempts${ext}`;

  try {
    await $`bun build ./src/cli.ts --compile --target bun-${platform} --outfile ${outdir}/${platform}/${filename} --minify`;

    // Create tarball
    await $`cd ${outdir} && tar -czf rempts-${version}-${platform}.tar.gz ${platform}`;
    await $`rm -rf ${outdir}/${platform}`;

    console.log(`✅ Built rempts-${version}-${platform}.tar.gz`);
  } catch (error) {
