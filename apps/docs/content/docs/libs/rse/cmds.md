...
1. open [src/docs/libs/sdk/sdk-mod.ts](https://github.com/reliverse/dler/blob/main/src/docs/libs/sdk/sdk-mod.ts) in your ide.
2. press `ctrl+a`, then `backspace`. run the command below and watch the magic happen:

```bash
bun tools:agg # shortcut for:
bun src/cli.ts tools --dev --tool agg --input src/docs/libs/sdk/sdk-impl --out src/docs/libs/sdk/sdk-mod.ts --recursive --named --strip src/docs/libs/sdk
```
...
# advanced example:
bun dler fs --mode copy --s ".temp/packages/*/lib/**/*" --d "src/docs/libs/sdk/sdk-impl/rules/external"
bun dler fs --mode rm --target "**/node_modules"
```
...
**usage example**:  
using `dler` to package [src/docs/libs/sdk](https://github.com/reliverse/dler/tree/main/src/docs/libs/sdk):
...
@reliverse/rempts's best friend. learn more in its [docs](https://github.com/reliverse/dler).

```bash
bun dler rempts
bun dler @reliverse/dler rempts --init cmd1 cmd2
```
