...
<Callout type="info">
  When using @reliverse/rempts, these utilities are automatically injected into your command handlers. You typically don't need to import them directly.
</Callout>
...
<Callout type="info">
  Type utilities are exported from `@reliverse/rempts`, not `@reliverse/rempts-utils`. They're designed to work with generated types for advanced type manipulation patterns.
</Callout>