...
<Tabs groupId="package-manager" defaultValue="bun">
  ...
  <Tab value="yarn" label="yarn">
    ```bash
    yarn create @reliverse/dler rempts my-cli
    ```
  </Tab>
  <Tab value="pnpm" label="pnpm">
    ```bash
    pnpm create @reliverse/dler rempts my-cli
    ```
  </Tab>
</Tabs>
...
```bash
bunx @reliverse/dler rempts my-cli --template advanced --package-manager pnpm
```
...
# Full GitHub format
bunx @reliverse/dler rempts my-cli --template github:username/repo
...
```bash
bunx @reliverse/dler rempts my-cli --template ./path/to/template
bunx @reliverse/dler rempts my-cli --template file:./path/to/template
```
...
Use @reliverse/dler rempts programmatically:
...
bunx @reliverse/dler rempts my-cli --offline

# Force offline mode (fail if not cached)
GIGET_FORCE_OFFLINE=1 bunx @reliverse/dler rempts my-cli
```
...
bunx @reliverse/dler rempts . --template advanced

# Create in custom path
bunx @reliverse/dler rempts my-cli --dir ~/projects/my-cli
```
...
```bash
bunx @reliverse/dler rempts weather-cli \
  --template advanced \
...
```bash
bunx @reliverse/dler rempts my-app \
...
```bash
bunx @reliverse/dler rempts api-cli \
  --template monorepo \
  --no-git \
  --no-install
```
