...
<Tabs groupId="package-manager" defaultValue="bun">
  ...
  <Tab value="yarn" label="yarn">
    ```bash
    yarn create rempts my-cli
    ```
  </Tab>
  <Tab value="pnpm" label="pnpm">
    ```bash
    pnpm create rempts my-cli
    ```
  </Tab>
</Tabs>
...
```bash
bunx rempts my-cli --template advanced --package-manager pnpm
```
...
```bash
bunx rempts my-cli --template ./path/to/template
bunx rempts my-cli --template file:./path/to/template
```
...
bunx rempts my-cli --offline

# Force offline mode (fail if not cached)
GIGET_FORCE_OFFLINE=1 bunx rempts my-cli
```
