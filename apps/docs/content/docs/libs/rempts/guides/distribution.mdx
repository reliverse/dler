...
bunx @reliverse/rempts build --targets native

# Cross-compile for specific architectures
bunx @reliverse/rempts build --targets darwin-arm64,linux-x64

# Build for complete platform matrix
bunx @reliverse/rempts build --targets all
...
bunx @reliverse/rempts build --targets native --minify --bytecode
```
...
bunx @reliverse/rempts build --targets darwin-arm64
  ...
  build:
    ...
    steps:
      ...
      - name: Cross-platform compilation
        run: bunx @reliverse/rempts build --targets all
...
// package.json
{
  ...
  "scripts": {
    "prepublishOnly": "bunx @reliverse/rempts build"
  }
}
```
...
COPY . .
RUN bunx @reliverse/rempts build --targets native
...
### Leveraging @reliverse/rempts release
...
bunx @reliverse/rempts release --version 2.0.0

# Release excluding NPM publication
bunx @reliverse/rempts release --no-npm

# Perform dry-run validation
bunx @reliverse/rempts release --dry-run
```
...
1. **Version Bump**
   ...
   # Manual version
   @reliverse/rempts release --version 2.0.0
   ```

2. **Build All Platforms**
   ```bash
   @reliverse/rempts build --targets all
   ```
