...
<Callout type="info">
  @rempts/generator has **minimal dependencies** and uses Babel for AST parsing to extract command metadata from your TypeScript files.
</Callout>
...
<Tabs groupId="package-manager" defaultValue="bun">
  <Tab value="bun" label="Bun">
    ```bash
    bun add @rempts/generator
    ```
  </Tab>
  <Tab value="npm" label="npm">
    ```bash
    npm install @rempts/generator
    ```
  </Tab>
  <Tab value="pnpm" label="pnpm">
    ```bash
    pnpm add @rempts/generator
    ```
  </Tab>
</Tabs>
...
```typescript
import { Generator } from '@rempts/generator'

const generator = new Generator({
  cmdsDir: './commands',
  outputFile: './.rempts/commands.gen.ts',
...
```typescript
import { CommandScanner } from '@rempts/generator'
...
```typescript
import { parseCommand } from '@rempts/generator'

const commandData = await parseCommand('./commands/greet.ts', './commands', './.rempts/commands.gen.ts')
...
```typescript
import { buildTypes } from '@rempts/generator'
...
```typescript
import { remptsCodegenPlugin } from '@rempts/generator'
...
| `outputFile` | `string` | `'./.rempts/commands.gen.ts'` | Output file for generated types |
...
- [Type Generation Guide](/libraries/rempts/guides/type-generation) - Complete guide to using generated types
- [rempts CLI](/libraries/rempts/packages/cli) - CLI toolchain with integrated generation
- [Configuration](/libraries/rempts/core-concepts/configuration) - Configure code generation
- [@rempts/core](/libraries/rempts/packages/core) - Core framework with type utilities
