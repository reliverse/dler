...
- ⚙️ **Configuration** - Flexible `rempts.config.ts` system
...
- `--output, -o` - Output file (default: `./.rempts/commands.gen.ts`)
...
rempts generate --commandsDir ./src/commands
```
...
Create a `rempts.config.ts` file in your project root to configure the CLI behavior:
...
1. `--entry` flag
2. `build.entry` in rempts.config.ts
...
cd packages/my-package && rempts dev
```

Configure workspaces in rempts.config.ts:
...
- `REMPTS_VERSION` - The version of the rempts CLI
...
- `--config, -c` - Path to config file (default: ./rempts.config.ts)
