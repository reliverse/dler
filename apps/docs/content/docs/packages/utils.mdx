...
<Callout type="info">
  @rempts/utils has **zero dependencies** and provides a custom ANSI-based implementation for all features. It respects NO_COLOR and CI environment variables automatically.
</Callout>
...
<Tabs groupId="package-manager" defaultValue="bun">
  ...
  <Tab value="npm" label="npm">
    ```bash
    npm install @rempts/utils
    ```
  </Tab>
  <Tab value="pnpm" label="pnpm">
    ```bash
    pnpm add @rempts/utils
    ```
  </Tab>
</Tabs>

<Callout type="info">
  When using @rempts/core, these utilities are automatically injected into your command handlers. You typically don't need to import them directly.
</Callout>
...
```typescript
import { prompt } from '@rempts/utils'
...
<Callout type="note">
  Multi-select is not yet implemented in the current version of @rempts/utils. Use multiple confirm prompts or select prompts as a workaround.
</Callout>
...
```typescript
import { spinner } from '@rempts/utils'
...
```typescript
import { colors } from '@rempts/utils'
...
```typescript
import { validate } from '@rempts/utils'
import { z } from 'zod'
...
```typescript
import { validateFields } from '@rempts/utils'
import { z } from 'zod'
...
## Type Utilities for Generated Types

When working with generated types from `@rempts/generator`, you can use advanced type utilities from `@rempts/core` for complex type manipulation:
...
<Callout type="info">
  Type utilities are exported from `@rempts/core`, not `@rempts/utils`. They're designed to work with generated types for advanced type manipulation patterns.
</Callout>