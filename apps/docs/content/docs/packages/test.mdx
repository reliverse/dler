...
<Tabs groupId="package-manager" defaultValue="bun">
  ...
  <Tab value="npm" label="npm">
    ```bash
    npm install -D @rempts/test
    ```
  </Tab>
  <Tab value="pnpm" label="pnpm">
    ```bash
    pnpm add -D @rempts/test
    ```
  </Tab>
</Tabs>
...
import { testCommand, expectCommand } from '@rempts/test'
```
...
```typescript
import { mockPromptResponses } from '@rempts/test'
...
```typescript
import { mockShellCommands } from '@rempts/test'
...
import { testCLI } from '@rempts/test'
...
```typescript
import { mockInteractive } from '@rempts/test'
...
```typescript
import { mergeTestOptions } from '@rempts/test'
