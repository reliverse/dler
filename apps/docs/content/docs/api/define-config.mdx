...
Defines configuration for Rempts CLI projects. Used in `rempts.config.ts` files.
...
<Callout type="tip">
  **Automatic Loading**: In most cases, you don't need to manually load config. `createCLI()` automatically loads from `rempts.config.ts` when called without arguments.
</Callout>
...
2. **Use TypeScript** - `rempts.config.ts` provides best type safety
