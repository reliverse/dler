---
title: Configuration
description: Configure your Rempts CLI project with rempts.config.ts
---

# Configuration

Configure your Rempts CLI project behavior with `rempts.config.ts`. **Configuration is automatically loaded** - you don't need to manually import or pass config to `createCLI()`.

## Automatic Configuration Loading

Rempts automatically loads configuration from `rempts.config.ts` when you call `createCLI()`:

```typescript
// cli.ts - No config needed!
import { createCLI } from '@rempts/core'

const cli = await createCLI()  // Automatically loads rempts.config.ts
...
```typescript
// rempts.config.ts - All configuration here
...
Create a `rempts.config.ts` file in your project root:
...
<Callout type="info">
  Type generation is automatically integrated with `rempts dev` and `rempts build` commands. Learn more in the [Type Generation Guide](/docs/guides/type-generation).
</Callout>
