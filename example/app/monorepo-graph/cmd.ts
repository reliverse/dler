...
import { graphCommand, type MonorepoContext } from "~/impl/monorepo/commands-mod";
import { commonEndActions, commonStartActions, getCurrentWorkingDirectory } from "~/mod";

export default defineCommand({
  meta: {
    name: "monorepo-graph",
    description: "Print the dependency graph of the monorepo",
  },
  args: defineArgs({
    ci: {
      type: "boolean",
  ...
  run: async ({ args }) => {
    ...
    const ctx: MonorepoContext = {
      ...
      cmdArgs: [], // No command args for graph
    };

    await graphCommand(ctx);
