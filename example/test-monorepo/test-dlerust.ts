...
async function testMonorepo() {
  ...
  const packageJsonGlobs = monorepo.packageGlobs.map((glob: string) => `${glob}/package.json`);
  const matches = await globby(packageJsonGlobs, { cwd: monorepo.root, absolute: true });

  const packages: Package[] = [];
  for (const packageJsonPath of matches) {
    const { readPackageJson } = await import("../../src/impl/monorepo/mod");
  ...
  console.log(`   ✓ Build order: ${buildOrder.map((p: Package) => p.name).join(" → ")}`);
  ...
  if (activePackage) {
    ...
    console.log(`   ✓ Dependencies: ${deps.map((p: Package) => p.name).join(", ")}`);
  } else {
