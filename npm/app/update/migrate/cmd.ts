// usage example: bun <npm/dler.ts | dler> update migrate --to-catalog --dry-run
// usage example: bun <npm/dler.ts | dler> update migrate --from-catalog --remove-catalog
...
import { isCatalogSupported } from "~/libs/sdk/sdk-impl/utils/pm/pm-catalog";
 ...
 */
function shouldSkipDependency(versionSpec: string): boolean {
  return (
 ...
 */
function extractDependencies(packageJson: PackageJson, packageJsonPath: string): DependencyEntry[] {
 ...
 */
async function replaceDependenciesWithCatalogRefs(
  packageJsonPath: string,
 ...
 */
async function restoreCatalogReferences(
  packageJsonPath: string,
  catalog: Record<string, string>,
): Promise<number> {
  try {
 ...
 */
async function updateRootWithCatalog(
 ...
 */
async function removeCatalogFromRoot(rootPackageJsonPath: string): Promise<void> {
  try {
    const packageJsonContent = await fs.readFile(rootPackageJsonPath, "utf8");
    const packageJson = JSON.parse(packageJsonContent) as PackageJson;
 ...
 */
async function migrateToCatalog(
  rootPath: string,
  dryRun = false,
  interactive = false,
 ...
 */
async function migrateFromCatalog(
  rootPath: string,
  removeCatalog = false,
  dryRun = false,
  interactive = false,
 ...
 */
function displayMigrationResults(results: MigrationResult[]): void {
  if (results.length === 0) {
...
export default defineCommand({
  ...
  args: defineArgs({
    "to-catalog": {
      type: "boolean",
      description:
        "Migrate workspace dependencies to workspaces.catalog and replace with catalog references",
    },
    "from-catalog": {
      type: "boolean",
