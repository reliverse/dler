 ...
 */
export async function findWorkspacePackageJsons(
	cwd: string,
): Promise<string[]> {
		...
		for (const [name, version] of Object.entries(deps)) {
			if (typeof version === "string" && isCatalogReference(version)) {
				...
				if (catalogVersion && packageJson.dependencies) {
					...
					relinka(
						"warn",
						`No catalog entry found for dependency: ${name} in ${packageJsonPath}`,
					);
				}
			}
		}
 ...
 */
export async function removeCatalogFromRoot(
	rootPackageJsonPath: string,
): Promise<void> {
	try {
		const packageJsonContent = await fs.readFile(rootPackageJsonPath, "utf8");
		...
		if (
			...
			if (
				...
				(Object.keys(packageJson.workspaces).length === 1 &&
...
export async function migrateToCatalog(
	...
	if (workspacePaths.length === 0) {
		throw new Error(
			"No workspace packages found. This command requires a monorepo setup.",
		);
	}
	...
	relinka(
		"log",
		`Found ${allDependencies.length} total dependency entries across workspaces`,
	);
	...
	relinka(
		"log",
		`  - ${mergeResult.bumped.length} dependencies bumped to newer versions`,
	);
	relinka(
		"log",
	...
	if (interactive && allDependencies.length > 0) {
		const selectedPackages = await multiselectPrompt({
			...
			options: [
				...
				...Array.from(new Set(allDependencies.map((d) => d.name))).map(
					(name) => ({
						label: name,
						value: name,
					}),
		...
		if (!selectedPackages.includes("all")) {
			dependenciesToMigrate = allDependencies.filter((dep) =>
				selectedPackages.includes(dep.name),
			);
		}
	}
	...
	if (dryRun) {
		...
		const uniqueDeps = Array.from(
			new Set(dependenciesToMigrate.map((d) => d.name)),
		);
	...
	for (const [packagePath, deps] of workspaceResults.entries()) {
		const replacedCount = await replaceDependenciesWithCatalogRefs(
			packagePath,
			deps,
		);
...
export async function migrateFromCatalog(
	...
	if (interactive && packagesWithCatalogRefs.length > 0) {
		...
		if (!selectedPackages.includes("all")) {
			...
			packagesToProcess = packagesWithCatalogRefs
				.map((pkg) => ({
					...pkg,
					catalogRefs: pkg.catalogRefs.filter((ref) =>
						selectedPackages.includes(ref),
					),
				}))
	...
	if (dryRun) {
		...
		if (removeCatalog) {
			relinka(
				"log",
				"Would also remove workspaces.catalog from root package.json",
			);
		}
	...
	for (const pkg of packagesToProcess) {
		...
		if (restoredCount > 0) {
			relinka(
				"log",
				`Restored ${restoredCount} catalog references in ${pkg.path}`,
			);
...
export function displayMigrationResults(results: MigrationResult[]): void {
	...
	relinka(
		"log",
		"Run your package manager install command to apply the changes",
	);
}
