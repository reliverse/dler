...
import { uploadToProvider } from "~/impl/upload/providers/providers-mod";
...
export default defineCommand({
  ...
  run: async ({ args }) => {
    const spinner = ora(`Generating image with prompt: "${args.prompt}"`).start();
    ...
    try {
      ...
      if ("uuid" in result && result.uuid) relinka("info", `UUID: ${result.uuid}\n`);
    } catch (error) {
