...
import { getProjectContent } from "~/impl/config/cfg-content";
import { PROJECT_FRAMEWORK_FILES } from "~/impl/config/cfg-default";
import { getRseConfigPath } from "~/impl/config/cfg-path";
import { readRseConfig } from "~/impl/config/cfg-read";
import type { DetectedProject, ProjectFramework } from "~/impl/types/mod";

export async function detectProjectFramework(
  projectPath: string,
): Promise<ProjectFramework | null> {
  // First check for specific package.json dependencies
  const packageJson = await getPackageJson(projectPath);
  if (!packageJson) {
    // If no package.json, check for configuration files
  ...
  // Collect all matching frameworks
  const matches: { type: ProjectFramework; priority: number }[] = [];
    ...
    for (const file of files) {
      ...
      if (filePath === "package.json" && packageName) {
        ...
        if (packageName in deps) {
          matches.push({ type: type as ProjectFramework, priority: 1 });
        }
      }
    }
  }
  ...
  // If no package matches, check for configuration files
  for (const [type, files] of Object.entries(PROJECT_FRAMEWORK_FILES)) {
    for (const file of files) {
      const [filePath = file] = file.split(":");
