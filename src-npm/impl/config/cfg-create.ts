...
import { injectSectionComments } from "~/impl/config/cfg-comments";
import { generateDefaultRulesForProject, getDefaultRseConfig } from "~/impl/config/cfg-def-utils";
import { DEFAULT_CONFIG_RSE } from "~/impl/config/cfg-default";
import { detectFeatures, getPackageJson } from "~/impl/config/cfg-detect";
import { getRseConfigPath } from "~/impl/config/cfg-path";
import { readRseConfig } from "~/impl/config/cfg-read";
import { rseSchema } from "~/impl/config/cfg-schema";
import {
  atomicWriteFile,
  cleanGitHubUrl,
...
import type { DeploymentService, RseConfig } from "~/impl/types/mod";
...
export async function writeRseConfig(
  ...
  if (configPath.endsWith(".ts")) {
    ...
    try {
      ...
      // - If dev is true, create an alias path to ~/impl/sdk-mod.js
