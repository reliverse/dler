...
// [copy] advanced example: `bun dler fs --mode copy --s ".temp/packages/*/lib/**/*" --d "src/impl/rules/external"`
...
import { createPerfTimer, getElapsedPerfTime } from "~/impl/sdk-mod";
import { prepareCLIFiles, safeRename } from "~/impl/utils/fs-rename";
    ...
    if (mode === "rm") {
      ...
      if (!finalDestination) {
        finalDestination = await inputPrompt({
          ...
          placeholder: "e.g., src/impl/rules/putout",
        });
      }
