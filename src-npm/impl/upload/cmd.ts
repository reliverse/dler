...
import { readFilesFromPaths, uploadToProvider } from "~/impl/upload/providers/providers-mod";
import { formatBytes } from "~/impl/upload/upload-utils";
...
export default defineCommand({
  ...
  run: async ({ args }) => {
    ...
    try {
      ...
      const inputFiles = (Array.isArray(args.files) ? args.files : [args.files]) as string[];
