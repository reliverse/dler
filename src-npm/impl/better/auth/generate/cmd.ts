...
import { getGenerator } from "~/impl/better/auth/(generators)";
import { getConfig } from "~/impl/better/auth/(utils)/get-config";
...
export default defineCommand({
  ...
  run: async ({ args }) => {
    ...
    if (schema.append || schema.overwrite) {
      ...
      if (!confirm) {
        confirm = await confirmPrompt({
          title: `The file ${schema.fileName} already exists. Do you want to ${re.yellow(
      ...
      if (confirm) {
        ...
        logger.success(
          `🚀 Schema was ${schema.overwrite ? "overwritten" : "appended"} successfully!`,
        );
        process.exit(0);
      } else {
    ...
    if (!confirm) {
      confirm = await confirmPrompt({
        title: `Do you want to generate the schema to ${re.yellow(schema.fileName)}?`,
      });
    }
    ...
    await fs.writeFile(options.output || path.join(cwd, schema.fileName), schema.code);
