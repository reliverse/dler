  ...
  handler: async ({ flags, spinner, colors, context }) => {
    ...
    if (follow) {
      ...
      const interval = setInterval(() => {
        ...
        console.log(`${relico.dim(timestamp)} ${levelColor(`[${logLevel!.toUpperCase()}]`)} ${relico.cyan(`[${serviceName}]`)} ${message}`)
      ...
      filteredLogs.forEach(log => {
        ...
        console.log(`${relico.dim(log.timestamp)} ${levelColor?.(`[${log.level.toUpperCase()}]`) || relico.gray(`[${log.level.toUpperCase()}]`)} ${relico.cyan(`[${log.service}]`)} ${log.message}`)
      })
    }
