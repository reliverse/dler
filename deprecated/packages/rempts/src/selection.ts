...
import { encode, toStringFromPtr } from "./utils";
...
>(
  ...
  const { selectedIndex, error } = JSON.parse(toStringFromPtr(returnedPtr)) as {
...
export async function confirmPrompt(
  ...
  const { confirmed, error } = JSON.parse(toStringFromPtr(returnedPtr)) as {
    confirmed: string;
    error: string;
  };
  if (error !== "") {
