// Performance benchmarks for @reliverse/mapkit

import { writeFile } from "node:fs/promises";
import { performance } from "node:perf_hooks";
import { fileURLToPath } from "node:url";
import { benchmarkEdgeCases } from "./impl/edge-cases";
import { benchmarkPMap } from "./impl/p-map";
import { benchmarkPMapIterable } from "./impl/p-map-iterable";
...
async function runBenchmarks(
  ...
  try {
    ...
    const pMapResults = await benchmarkPMap(benchmark);
    sections.push({
      title: "pMap Performance",
      results: pMapResults,
    });
    allResults.push(...pMapResults);

    const iterableResults = await benchmarkPMapIterable(benchmark);
    sections.push({
      title: "pMapIterable Performance",
      results: iterableResults,
    });
    allResults.push(...iterableResults);

    const edgeCaseResults = await benchmarkEdgeCases(benchmark);
    sections.push({
