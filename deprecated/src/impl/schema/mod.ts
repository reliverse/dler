export interface ReliverseConfig {
  ...
  /**
   * Configuration for CLI functionality per package:
   * - Key: Package name (use "." or "main" for main project)
   * - Value: Object with enabled flag and scripts map
   ...
   *
   * **The source scripts should be in your "commonEntrySrcDir" directory (by default "src")**
   *
   * @example
   * {
   *   ".": { enabled: true, scripts: { "mycli": "cli.ts" } },
   *   "@myorg/package-a": { enabled: true, scripts: { "pkg-a": "cli.ts" } },
   *   "@myorg/package-b": { enabled: false, scripts: {} }
   * }
   *
   ...
   * If not specified, will use the commonEntryFile from the commonEntrySrcDir.
   *
   * @default undefined (uses commonEntryFile)
   */
  ...
  /**
   * Internal flag set for workspace package builds to adjust output layout.
...
export const DEFAULT_CONFIG_RELIVERSE: ReliverseConfig = {
  ...
  commonIsCLI: { ".": { enabled: false, scripts: {} } },
