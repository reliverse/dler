  ...
  if (options.frontend && options.frontend.length > 0) {
    if (options.frontend.includes("none")) {
      if (options.frontend.length > 1) {
        relinka("fatal", `Cannot combine 'none' with other frontend options.`);
        process.exit(1);
      }
      config.frontend = [];
    } else {
  ...
  if (options.addons && options.addons.length > 0) {
    if (options.addons.includes("none")) {
      if (options.addons.length > 1) {
        relinka("fatal", `Cannot combine 'none' with other addons.`);
        process.exit(1);
      }
      config.addons = [];
    } else {
  ...
  if (options.examples && options.examples.length > 0) {
    if (options.examples.includes("none")) {
      if (options.examples.length > 1) {
        relinka("fatal", "Cannot combine 'none' with other examples.");
        process.exit(1);
      }
      config.examples = [];
    } else {
