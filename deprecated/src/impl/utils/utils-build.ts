...
import type { NpmOutExt, ReliverseConfig, Sourcemap } from "~/impl/schema/mod";
...
export async function renameEntryFile(
  ...
  if (isJsr) {
    ...
    if (originalEntryFileBasename.endsWith(".ts")) {
      ...
      targetEntryBasename = config.commonEntryFile; // Use commonEntryFile from config
    } else {
      // Handle cases where entry might be .js or .tsx -> outputExt
      sourceEntryBasename = `${originalBasenameNoExt}.${outputExt}`; // e.g., index.js
      targetEntryBasename = `${config.commonEntryFile.replace(/\.ts$/, `.${outputExt}`)}`; // e.g., mod.js
    }
  } else {
    ...
    targetDeclarationBasename = `${config.commonEntryFile.replace(/\.ts$/, ".d.ts")}`; // Target declaration name
  }
