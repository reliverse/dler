...
export async function library_buildFlow(
  ...
  if (
    ...
    relinka(
      "verbose",
      "Skipping libs build as libsActMode is set to 'main-project-only'",
    );
    return;
  }
...
export async function library_pubFlow(
  ...
  if (
    ...
    relinka(
      "verbose",
      "Skipping libs publish as libsActMode is set to 'main-project-only'",
    );
    return;
  }
  ...
  await libraries_publish(
    ...
    config.commonDisableSpinner === false,
  );
}
...
export async function libraries_publish(
  ...
  if (!libsList || Object.keys(libsList).length === 0) {
    relinka(
      "verbose",
      "No lib configs found in config, skipping libs publish.",
    );
    return;
  }
        ...
        if (!commonPubPause && !libConfig.libPubPause) {
          ...
          if (publishSpinner) {
            publishSpinner.succeed(
              `${libName} published to ${effectivePubRegistry} successfully`,
            );
          }
