import { dirname as pathDirname, relative } from "@reliverse/pathkit";
import { relinka } from "@reliverse/relinka";
import { rmdir, symlink, warn } from "~/impl/build/providers/utils";
import type {
  BuildContext,
  MkdistBuildEntry,
  MkdistOptions,
...
export async function mkdistBuild(ctx: BuildContext): Promise<void> {
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      relinka(
        "verbose",
        `[mkdist] Entry ${processedEntries} completed in ${duration}ms`,
      );

      totalWrittenFiles += output.writtenFiles.length;

      ctx.buildEntries.push({
        chunks: output.writtenFiles.map((p: string) =>
          relative(ctx.options.outDir, p),
        ),
        path: distDir,
