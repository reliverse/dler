...
import { createPerfTimer, getElapsedPerfTime } from "~/impl/utils/utils-perf";
...
export async function unifiedBuild(
  ...
  relinka(
    "verbose",
    `Processing build for source directory: ${inputSourceDir}`,
  );
  relinka(
    "verbose",
    `Output directory: ${outDir}, Is CLI: ${commonIsCLI.enabled}, Is Library: ${isLib}`,
  );
  ...
  const _buildConfig: UnifiedBuildConfig | UnifiedBuildConfig[] =
    (await jiti.import(inputConfig?.config || "reliverse.ts", {
      default: true,
  ...
  // Invoke build for every build config defined in `reliverse.ts`
...
async function _build(
  ...
  relinka(
    "verbose",
    `Build options: clean=${options.clean}, parallel=${options.parallel}`,
  );
  relinka(
    "verbose",
    `Declaration files: ${options.declaration ? "enabled" : "disabled"}`,
  );
...
`,
  ...
  const activeTasks = buildTasks.filter(({ task }) => {
    if (task === typesBuild)
      return options.entries.some((e) => e.builder === "untyped");
    if (task === mkdistBuild)
      return options.entries.some((e) => e.builder === "mkdist");
    if (task === rollupBuild)
      return options.entries.some((e) => e.builder === "rollup");
    if (task === copyBuild)
      return options.entries.some((e) => e.builder === "copy");
    return false;
  });
  ...
  if (activeTasks.length === 0) {
    ...
    if (options.parallel) {
      relinka(
        "verbose",
        `Running ${activeTasks.length} build tasks in parallel...`,
      );
      await Promise.all(
