...
import { determineDistName } from "~/impl/utils/utils-determine";
import {
  copyRootFile,
  deleteSpecificFiles,
  getDirectorySize,
...
import { library_createPackageJSON } from "~/impl/utils/utils-package-json-libraries";
...
interface NpmBuildOptions {
  ...
  commonEntrySrcDir: string; // Base source directory containing libs (e.g., "src")
}
...
async function library_buildNpmDist(
  ...
  const { npmOutDir, distNpmBuilder, commonEntrySrcDir } = npmOptions;
  ...
  relinka(
    "verbose",
    `[NPM:${distName}] Initializing NPM build for ${libName}...`,
  );
  ...
  const { libSpecificSrcDir, libDirName } = await determineNpmSourceDirectory(
    libName,
    libMainFile,
    path.resolve(PROJECT_ROOT, commonEntrySrcDir),
    libsList,
    distName,
  );
...
async function library_buildDistributionTarget(
  ...
  const commonStepsParams: CommonStepsParams = {
    commonEntryFileName: path.basename(libMainFile),
    outputDirRoot,
...
async function library_performCommonBuildSteps(
  ...
  relinka(
    "verbose",
    `${logPrefix} Performing common steps in ${outputDirRoot}. Entry file: ${commonEntryFileName}. ${isJsr ? `JSR out files ext: ${distJsrOutFilesExt}` : `NPM out files ext: ${unifiedBundlerOutExt}`}`,
  );
...
async function determineNpmSourceDirectory(
  ...
  commonEntrySrcDirResolved: string, // Absolute path to the base dir containing libs
  libsList: Record<string, LibConfig>,
  ...
  const libConfig = libsList?.[libName];
  let libSpecificSrcDir = commonEntrySrcDirResolved; // Default
  ...
  if (libConfig?.libMainFile) {
    ...
    relinka(
      "verbose",
      `${logPrefix} No libMainFile configured. Using core source directory: ${commonEntrySrcDirResolved}`,
    );
  }
  ...
  if (!(await fs.pathExists(libSpecificSrcDir))) {
    relinka(
      "warn",
      `${logPrefix} Determined source directory does not exist: ${libSpecificSrcDir}. Falling back to core source dir: ${commonEntrySrcDirResolved}`,
    );
    libSpecificSrcDir = commonEntrySrcDirResolved; // Fallback
