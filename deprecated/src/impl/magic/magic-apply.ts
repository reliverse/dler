    ...
    if (await fs.pathExists(distLibsPath)) {
      ...
      for (const libDir of libDirs) {
        if (libDir.isDirectory()) {
          ...
          // Add paths in the format: /libs/{libName}/app/spell/ and /{libName}/app/spell/
          paths.push(`/libs/${libName}/app/spell/`);
          paths.push(`/${libName}/app/spell/`);

          // Check what registry directories exist for this lib
          const libPath = path.join(distLibsPath, libName);
          try {
            const registryDirs = await fs.readdir(libPath, {
              withFileTypes: true,
            });
            ...
            for (const registryDir of registryDirs) {
              if (
                ...
                paths.push(`/libs/${libName}/${registryName}/app/spell/`);
                paths.push(`/${libName}/${registryName}/app/spell/`);
              }
            }
          } catch (error) {
            if (DEBUG_MODE) {
