...
export default defineCommand({
  ...
  args: defineArgs({
    ...
    lib: {
      type: "string",
      description:
        "Library name to process (e.g., sdk, cfg). Only valid with dist-libs target.",
    },
    concurrency: {
      type: "number",
...
Examples:
  ...
  For more information, see README.md#15-magic`,
    ...
    if (lib) {
      ...
      if (
        targets.some(
          (t: string) => t.startsWith("dist-libs/") && t !== `dist-libs/${lib}`,
        )
      ) {
        throw new Error(
    ...
    try {
      ...
      const distTargets = finalTargets.filter((t: string) =>
        t.startsWith("dist-"),
      );
      const customTargets = finalTargets.filter(
        (t: string) => !t.startsWith("dist-"),
      );
      ...
      if (customTargets.length > 0) {
        ...
        for (const target of customTargets) {
          console.log(
            `  - ${target} (will process magic directives directly in target files)`,
          );
        }
      }
