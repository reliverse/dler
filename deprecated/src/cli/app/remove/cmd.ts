...
export default defineCommand({
  meta: {
    ...
    version: "1.1.0",
    description:
      "Remove dependencies. Usage example: `dler remove package-name`",
  },
  args: defineArgs({
    action: {
      type: "string",
    ...
    filter: {
      type: "array",
      description:
        "Filter workspaces to operate on (e.g., 'pkg-*', '!pkg-c', './packages/pkg-*')",
    },
    "from-catalog": {
      type: "string",
      description:
        "Remove dependencies from catalog (e.g., 'default', 'testing', 'build')",
    },
    "catalog-name": {
      type: "string",
      description:
        "Name of the catalog to remove dependencies from (used with --from-catalog)",
    },
    standalone: {
      type: "boolean",
  ...
  async run({ args }) {
    ...
    if (fromCatalog && name) {
      ...
      const dependencies = Array.isArray(name)
        ? (name as string[])
        : [name as string];
      const catalogType = fromCatalog === "default" ? "catalog" : "catalogs";
      const actualCatalogName =
        fromCatalog === "default" ? undefined : catalogName || fromCatalog;
    ...
    switch (action) {
      ...
      default: {
        ...
        if (!action || action === "remove") {
          ...
          relinka.verbose(
            "Available actions: remove, rm, uninstall, un, delete, del",
          );
          return process.exit(1);
        }
      }
...
async function getCurrentAppsJson(
  appsPath: string,
  binaryKey: string,
...
async function removeFromAppsJson(
  appsPath: string,
  binaryKey: string,
): Promise<void> {
  try {
...
async function removeFromPath(installDir: string): Promise<void> {
  ...
  if (os === "win32") {
    ...
    try {
      ...
      relinka(
        "info",
        `Please manually remove ${installDir} from your PATH environment variable`,
      );
    }
  } else {
    // Unix-like systems
