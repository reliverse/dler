...
async function handleDirectoryConflict(currentPathInput: string): Promise<{
  ...
  while (true) {
    ...
    relinka(
      "warn",
      `Directory "${re.yellow(currentPathInput)}" already exists and is not empty.`,
    );
...
async function setupProjectDirectory(
  ...
  if (shouldClearDirectory) {
    ...
    try {
      ...
      s.stopAndPersist({
        text: re.red(`Failed to clear directory "${finalResolvedPath}".`),
      });
...
async function createProjectHandler(
  ...
  try {
    ...
    outro(
      re.magenta(
        `Project created successfully in ${re.bold(elapsedTimeInSeconds)} seconds!`,
      ),
    );
  } catch (error) {
    relinka("error", String(error));
