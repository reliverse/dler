...
export default defineCommand({
  ...
  args: defineArgs({
    template: {
      type: "positional",
      description:
        "Template name for project creation (e.g., react-app, next-app)",
      required: false,
    },
    mode: {
      type: "string",
    ...
    fileType: {
      type: "string",
      description:
        "File type to create (e.g. 'md:README') - only for files mode",
    },
    destDir: {
      type: "string",
    ...
    multiple: {
      type: "boolean",
      description:
        "Whether to select multiple file types from the library - only for files mode",
    },
    parallel: {
      type: "boolean",
    ...
    concurrency: {
      type: "string",
      description:
        "Concurrency limit if parallel is true - only for files mode",
      default: "4",
    },
    cwd: {
      type: "string",
