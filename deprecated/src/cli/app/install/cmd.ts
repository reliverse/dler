...
export default defineCommand({
  ...
  args: defineArgs({
    ...
    "as-catalog": {
      type: "string",
      description:
        "Install dependencies as catalog entries (e.g., 'default', 'testing', 'build')",
    },
    "catalog-name": {
      type: "string",
      description:
        "Name of the catalog to add dependencies to (used with --as-catalog)",
    },
  }),
  async run({ args }) {
    ...
    if (asCatalog && name) {
      ...
      const actualCatalogName =
        asCatalog === "default" ? undefined : catalogName || asCatalog;
    ...
    switch (action) {
      ...
      default: {
        ...
        if (!action || action === "install") {
          console.log(
            "DEBUG: default install case, name:",
            name,
            "options:",
            options,
          );
