...
export default defineCommand({
  ...
  args: defineArgs({
    ...
    source: {
      type: "string",
      description:
        "Source file or directory to complete the operation (supports glob patterns)",
      alias: "s",
    },
    destination: {
      type: "string",
  ...
  async run({ args }) {
    ...
    if (mode === "rm") {
      ...
      await pMap(
        ...
        async (match) => {
          ...
          try {
            ...
            relinka(
              "error",
              `Failed to remove '${resolvedPath}': ${errorMessage}`,
            );
          }
        },
        { concurrency },
      ...
      if (removedCount > 0) {
        relinka(
          "verbose",
          `Successfully removed ${removedCount} item(s) matching: ${target}`,
        );
      } else {
      ...
      if (gitignore) {
        try {
          ...
          relinka(
            "error",
            ".gitignore not found or unreadable, but --gitignore was specified.",
          );
          process.exit(1);
        }
      }
      ...
      try {
        ...
        relinka(
          "verbose",
          `Successfully renamed '${source}' to '${destination}'`,
        );
      } catch (error: unknown) {
        const errorMessage =
          error instanceof Error ? error.message : String(error);
