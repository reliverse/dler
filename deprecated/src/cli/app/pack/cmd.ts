...
export default defineCommand({
  meta: {
    ...
    version: "1.2.0",
    description:
      "Pack templates into TS modules or --unpack them back to files",
  },
  args: defineArgs({
    input: {
      type: "positional",
    ...
    update: {
      ...
      default: true,
      description:
        "Update existing templates and add new ones (pack mode only)",
    },
    files: {
      type: "string",
  ...
  async run({ args }) {
    ...
    if (args.unpack) {
      ...
      try {
        ...
        if (!stat.isFile() || !aggrPath.endsWith(".ts")) {
          throw new Error(
            "Input for --unpack must be a TypeScript aggregator file (*.ts)",
          );
        }
      } catch (err) {
    ...
    const filesToUpdate = args.files
      ? new Set(args.files.split(",").map((f) => f.trim()))
      : null;
    ...
    try {
      ...
      if (files.length > 0) {
        if (!args.force && !args.update) {
          relinka(
            "error",
            `Error: Output directory '${outDir}' already exists and is not empty.`,
          );
          relinka(
            "error",
        ...
        if (args.update) {
          ...
          try {
            ...
            relinka(
              "verbose",
              `Error details: ${(loadError as Error).message}`,
            );
          }
        }
      }
    ...
    const templateDirs = (
      await fs.readdir(dirToProcess, { withFileTypes: true })
    )
    ...
    for (const tplName of templateDirs) {
      ...
      code.push(
        `  description: "Template generated from ${allFiles.length} files",`,
      );
      ...
      fileEntries.forEach(([rel, meta], index) => {
        ...
        if (meta.jsonComments)
          code.push(
            `        jsonComments: ${JSON.stringify(meta.jsonComments, null, 2)},`,
          );
        if (meta.metadata) {
        ...
        if (meta.type === "binary") {
          ...
          code.push(
            `        content: \`${escapeTemplateString(meta.content as string)}\`,`,
          );
          code.push('        type: "text",');
        } else {
          ...
          try {
            ...
            clone = JSON.parse(JSON.stringify(meta.content)) as Record<
              string,
              unknown
            >;
            ...
            relinka(
              "error",
              `Failed to process JSON content for ${rel}: ${error}`,
            );
          }
        ...
        try {
          ...
          if (existingContent !== newContent) {
            if (filesToUpdate) {
              relinka(
                "verbose",
                `Updating specific files in template: ${tplName}`,
              );
            } else {
      ...
      const filesWithErrors = Object.entries(filesRecord).filter(
      ...
      /** aggregate */
      aggregatedImports.push(
        `import { ${varName} } from "./${TPLS_DIR}/${tplName}";`,
      );
    ...
    relinka(
      "success",
      `Packed ${templateDirs.length} templates into ${modFile}:`,
    );
    ...
    const binaryCount = Object.values(existingTemplates).reduce(
      (count, template) =>
        count +
        Object.values(template.config.files).filter(
    ...
    if (binaryCount > 0) {
      relinka(
        "info",
        `  - ${TPLS_DIR}/${BINARIES_DIR}/* (${binaryCount} binary files)`,
      );
    }
  },
});
