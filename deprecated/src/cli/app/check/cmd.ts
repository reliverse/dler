...
export default defineCommand({
  ...
  args: defineArgs({
    ...
    "no-exit": {
      type: "boolean",
      description:
        "don't exit with error code on issues (useful for pre-build checks)",
    },
    "no-progress": {
      type: "boolean",
      description:
        "don't show progress information (useful for pre-build checks)",
    },
        ...
        : (current: number, total: number) => {
            if (current % 10 === 0 || current === total) {
              process.stdout.write(
                `\r  progress: ${current}/${total} files...`,
              );
            }
          };
