...
export default defineCommand({
  ...
  args: defineArgs({
    action: {
      type: "string",
      description:
        "Action to perform: detect, run, exec, or package name to run directly",
      required: true,
    },
    name: {
      type: "positional",
    ...
    args: {
      type: "string",
      description:
        "Additional arguments to pass to the package (space-separated)",
    },
    global: {
      type: "boolean",
      alias: "g",
  ...
  async run({ args }) {
    ...
    switch (action) {
      ...
      default: {
        ...
        relinka.verbose(
          "Available actions: detect, run, exec, or package name to run directly",
        );
        return process.exit(1);
      }
    }
...
async function runPackage({
  ...
  try {
    ...
    const forcedPmFlags = Object.entries(forcePm).filter(
      ([_, isForced]) => isForced,
    );
