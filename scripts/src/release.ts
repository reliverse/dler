    ...
    try {
      for (const pkg of packages) {
        ...
        if (modifiedPackages.has(packageDir)) {
          try {
            const { stdout: lastVersion } =
              await $`git show ${lastRelease}:packages/${packageDir}/package.json | grep '"version"' | head -1 | cut -d'"' -f4`.quiet();
...
async function promptForReleaseOptions(
  ...
  if (modifiedPackages.length > 0) {
    ...
    modifiedPackages.forEach((pkg) => {
      const gitProgression =
        pkg.lastVersion && pkg.lastVersion !== pkg.version
