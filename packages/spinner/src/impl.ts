import { type ColorName, re } from "@reliverse/dler-colors";
...
export async function oraPromise<T>(
  ...
  const actionIsPromise =
    !actionIsFunction && typeof (action as Promise<T>).then === "function";
