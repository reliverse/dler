...
const defaultStderr: NodeJS.WriteStream =
...
function getDefaultEnabled(
  stream: NodeJS.WriteStream = defaultStderr,
): boolean {
  const disabledByEnv =
    process.env["CLI_NO_SPINNER"] === "1" ||
    process.env["CLI_NO_COLOR"] === "1";
...
export function isSpinnerEnabled(options?: {
  ...
  const stream =
    (options?.stream as NodeJS.WriteStream | undefined) ?? defaultStderr;
...
export function createSpinner(input?: string | SpinnerOptions): SimpleSpinner {
  ...
  const isEnabled =
    base.isEnabled ?? (respectEnv ? getDefaultEnabled(stream) : true);
  const isSilent = base.isSilent ?? false;

  const resolvedColor = (base.color ??
    defaultSpinnerOptions.color ??
    "cyan") as NonNullable<OraOptions["color"]>;
  const resolvedSpinner = (base.spinner ??
    defaultSpinnerOptions.spinner ??
    "dots") as NonNullable<OraOptions["spinner"]>;
  const resolvedHideCursor =
    base.hideCursor ?? defaultSpinnerOptions.hideCursor ?? true;
  const resolvedIndent = base.indent ?? defaultSpinnerOptions.indent ?? 0;
  const resolvedDiscardStdin =
    base.discardStdin ?? defaultSpinnerOptions.discardStdin ?? true;
  ...
  const dim = colorsEnabled
    ? (themeObj.dim ?? (re as any).dim ?? identity)
    : identity;
  const info = colorsEnabled
    ? (themeObj.info ?? (re as any).cyan ?? identity)
    : identity;
  const success = colorsEnabled
    ? (themeObj.success ?? (re as any).green ?? identity)
    : identity;
  const error = colorsEnabled
    ? (themeObj.error ?? (re as any).red ?? identity)
    : identity;
  const progress = colorsEnabled
    ? (themeObj.progress ?? (re as any).cyan ?? identity)
    : identity;
  const rate = colorsEnabled
    ? (themeObj.rate ?? (re as any).cyan ?? identity)
    : identity;
  const bytesColor = colorsEnabled
    ? (themeObj.bytes ?? (re as any).cyan ?? identity)
    : identity;
...
export async function withSpinner<T>(
  ...
  try {
    ...
    const style =
      ((spinner as any).__successStyler as
        | ((s: string) => string)
        | undefined) ?? identity;
    ...
    const style =
      ((spinner as any).__failStyler as ((s: string) => string) | undefined) ??
      identity;
...
export function updateSpinnerText(
  ...
  const styleText =
    ((spinner as any).__textStyler as ((s: string) => string) | undefined) ??
    identity;
  const stylePrefix =
    ((spinner as any).__prefixStyler as ((s: string) => string) | undefined) ??
    identity;
  const styleSuffix =
    ((spinner as any).__suffixStyler as ((s: string) => string) | undefined) ??
    identity;
...
export function createTimedSpinner(
  input?: string | Omit<SpinnerOptions, "showTiming">,
): {
  spinner: SimpleSpinner;
  getElapsed: () => number;
  ...
  return {
    ...
    succeedWithTiming: (text?: string) => {
      ...
      const timing = prettyMilliseconds(elapsed, { compact: true });
      const dim =
        ((spinner as any).__reTheme?.dim as
          | ((s: string) => string)
          | undefined) ?? identity;
      const successStyle =
        ((spinner as any).__successStyler as
    ...
    failWithTiming: (text?: string) => {
      ...
      const dim =
        ((spinner as any).__reTheme?.dim as
          ...
          | undefined) ?? identity;
      const failStyle =
        ((spinner as any).__failStyler as
          | ((s: string) => string)
          | undefined) ?? identity;
      const message = text
        ? `${failStyle(text)} ${dim(`(failed after ${timing})`)}`
        : undefined;
...
export async function withEnhancedSpinner<T>(
  ...
  const enhancedSpinner = Object.assign(baseSpinner, {
    updateText: (
      text: string,
      updateOptions?: { prefix?: string; suffix?: string },
    ) => {
...
export function createBuildSpinner(
  ...
  return {
    ...
    complete: (message?: string) => {
      const successStyle =
        ((spinner as any).__successStyler as
          ...
          | undefined) ?? identity;
      spinner.succeed(
        successStyle(message ?? `${operation} completed successfully!`),
      );
    },
    ...
    updateProgress: (step: string) => {
      const info =
        ((spinner as any).__reTheme?.info as
          | ((s: string) => string)
          | undefined) ?? identity;
  ...
  return {
    ...
    updateProgress: (bytesProcessed: number, fileName?: string) => {
      ...
      if (showBytes) {
        if (totalBytes) {
          ...
          const bytesColor =
            ((spinner as any).__reTheme?.bytesColor as
              | ((s: string) => string)
              | undefined) ?? identity;
          const percent =
            ((spinner as any).__reTheme?.percent as
              | ((s: string) => string)
              | undefined) ?? identity;
    ...
    updateRate: (bytesPerSecond: number) => {
      if (showRate) {
        ...
        const rate =
          ((spinner as any).__reTheme?.rate as
            | ((s: string) => string)
            | undefined) ?? identity;
    ...
    complete: (message?: string) => {
      ...
      const successStyle =
        ((spinner as any).__successStyler as
          | ((s: string) => string)
          | undefined) ?? identity;
      const successMessage = successStyle(
 ...
 */
export function formatSpinnerTiming(
  startTime: number,
  ...
  return prettyMilliseconds(
    elapsed,
    pmOptions as Parameters<typeof prettyMilliseconds>[1],
  );
}
 ...
 */
export function formatSpinnerBytes(
  bytes: number,
 ...
 */
export function formatSpinnerElapsed(
  elapsed: number,
  options?: { verbose?: boolean },
): string {
  const pmOptions: Record<string, unknown> = { compact: !options?.verbose };
    ...
    complete: (message?: string, totalBytesTransferred?: number) => {
      ...
      const successStyle =
        ((spinner as any).__successStyler as
          | ((s: string) => string)
          | undefined) ?? identity;
      let successMessage = successStyle(
        message ?? `${operation} completed successfully`,
      );
