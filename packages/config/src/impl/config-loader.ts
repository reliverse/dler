...
import { remptsConfigSchema } from "@reliverse/rempts";
import type { BuildConfig } from "./build";
import type { PublishConfig } from "./publish";

// Type for loaded config with defaults applied by @reliverse/dler rempts schema
// Note: The build field is overridden to use the extended BuildConfig instead of the simple @reliverse/dler rempts build config
  ...
  for (const configName of CONFIG_NAMES) {
    ...
    if (existsSync(configPath)) {
      try {
        ...
        // The @reliverse/dler rempts schema applies defaults, so these fields should always be present
