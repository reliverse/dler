// Performance benchmarks for @reliverse/config

import { writeFile } from "node:fs/promises";
import { performance } from "node:perf_hooks";
import { fileURLToPath } from "node:url";
import { benchmarkConfigMerging } from "./impl/config-merging";
import { benchmarkConfigResolution } from "./impl/config-resolution";
import { benchmarkPatternMatching } from "./impl/pattern-matching";
import { benchmarkVersionOperations } from "./impl/version-operations";
...
async function runBenchmarks(outputPath: string = defaultOutputPath): Promise<void> {
  ...
  try {
    ...
    const versionResults = await benchmarkVersionOperations(benchmark);
    sections.push({
      title: "Version Operations Performance",
      results: versionResults,
    });
    allResults.push(...versionResults);

    const resolutionResults = await benchmarkConfigResolution(benchmark);
    sections.push({
      title: "Configuration Resolution Performance",
      results: resolutionResults,
    });
    allResults.push(...resolutionResults);

    const mergingResults = await benchmarkConfigMerging(benchmark);
    sections.push({
      title: "Configuration Merging Performance",
      results: mergingResults,
    });
    allResults.push(...mergingResults);

    const patternResults = await benchmarkPatternMatching(benchmark);
    sections.push({
