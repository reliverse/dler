// Performance benchmarks for @reliverse/bump

import { writeFile } from "node:fs/promises";
import { performance } from "node:perf_hooks";
import { fileURLToPath } from "node:url";
import { benchmarkVersionBumping } from "./impl/version-bumping";
import { benchmarkVersionParsing } from "./impl/version-parsing";
import { benchmarkVersionQueries } from "./impl/version-queries";
...
async function runBenchmarks(outputPath: string = defaultOutputPath): Promise<void> {
  ...
  try {
    ...
    const parsingResults = await benchmarkVersionParsing(benchmark);
    sections.push({
      title: "Version Parsing Performance",
      results: parsingResults,
    });
    allResults.push(...parsingResults);

    const bumpingResults = await benchmarkVersionBumping(benchmark);
    sections.push({
      title: "Version Bumping Performance",
      results: bumpingResults,
    });
    allResults.push(...bumpingResults);

    const queryResults = await benchmarkVersionQueries(benchmark);
    sections.push({
      title: "Version Query Performance",
