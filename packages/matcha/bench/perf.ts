// Performance benchmarks for @reliverse/matcha

import { writeFile } from "node:fs/promises";
import { performance } from "node:perf_hooks";
import { fileURLToPath } from "node:url";
import { benchmarkCoreMatching } from "./impl/core-matching";
import { benchmarkFiltering } from "./impl/filtering";
import { benchmarkMatcherCreation } from "./impl/matcher-creation";
import { benchmarkPatternCompilation } from "./impl/pattern-compilation";
...
async function runBenchmarks(outputPath: string = defaultOutputPath): Promise<void> {
  ...
  try {
    ...
    const coreResults = await benchmarkCoreMatching(benchmark);
    sections.push({
      title: "Core Matching Performance",
      results: coreResults,
    });
    allResults.push(...coreResults);

    const matcherResults = await benchmarkMatcherCreation(benchmark);
    sections.push({
      title: "Matcher Creation Performance",
      results: matcherResults,
    });
    allResults.push(...matcherResults);

    const compilationResults = await benchmarkPatternCompilation(benchmark);
    sections.push({
      title: "Pattern Compilation Performance",
      results: compilationResults,
    });
    allResults.push(...compilationResults);

    const filteringResults = await benchmarkFiltering(benchmark);
    sections.push({
      title: "Filtering Operations Performance",
