// Performance benchmarks for @reliverse/build

import { writeFile } from "node:fs/promises";
import { performance } from "node:perf_hooks";
import { fileURLToPath } from "node:url";
import { benchmarkCache } from "./impl/cache";
import { benchmarkDependencyTracking } from "./impl/dependency-tracking";
import { benchmarkEntryPointDetection } from "./impl/entry-point-detection";
import { benchmarkPackageDiscovery } from "./impl/package-discovery";
import { benchmarkPackageFiltering } from "./impl/package-filtering";
import { benchmarkPackageInfoResolution } from "./impl/package-info-resolution";
...
async function runBenchmarks(outputPath: string = defaultOutputPath): Promise<void> {
  ...
  try {
    ...
    const discoveryResults = await benchmarkPackageDiscovery(benchmark);
    sections.push({
      title: "Package Discovery Performance",
      results: discoveryResults,
    });
    allResults.push(...discoveryResults);

    const entryPointResults = await benchmarkEntryPointDetection(benchmark);
    sections.push({
      title: "Entry Point Detection Performance",
    ...
    allResults.push(...infoResolutionResults);

    const filteringResults = await benchmarkPackageFiltering(benchmark);
    sections.push({
      title: "Package Filtering Performance",
      results: filteringResults,
    });
    allResults.push(...filteringResults);

    const cacheResults = await benchmarkCache(benchmark);
    sections.push({
      title: "Cache Operations Performance",
      results: cacheResults,
    });
    allResults.push(...cacheResults);

    const dependencyResults = await benchmarkDependencyTracking(benchmark);
    sections.push({
      title: "Dependency Tracking Performance",
