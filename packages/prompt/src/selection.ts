...
export function selectPrompt(items: SelectionItem[], options: SelectionOptions = {}): SelectionReturn {
...
export function multiselectPrompt(items: SelectionItem[], options: SelectionOptions = {}): MultiselectReturn {
  const stringifiedItems = JSON.stringify(items.map((item) => {
    return {
      text: item.text,
      description: item.description || ''
  ...
  const returnedPtr = symbols.CreateMultiselect(
    ptr(encode(stringifiedItems)),
    ptr(encode(options.headerText || 'Select items: ')),
    ptr(encode(options.footerText || '')),
    options.perPage || 5
  )
