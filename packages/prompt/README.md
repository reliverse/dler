# 📃 dler-prompt • powerful js/ts cli builder

[sponsor](https://github.com/sponsors/blefnk) — [discord](https://discord.gg/Pb8uKbwpsJ) — [repo](https://github.com/reliverse/dler) — [npm](https://npmjs.com/@reliverse/dler-prompt)

> @reliverse/dler-prompt is a modern, type-safe toolkit for building delightful cli experiences. it's fast, flexible, and made for developer happiness. file-based commands keep things simple—no clutter, just clean and easy workflows. this is how cli should feel.
...
- 🫂 dler-prompt keeps you from fighting with your CLI tool
- 🏎️ prompt engine that *feels* modern — and actually is
- ✨ dler-prompt is your end-to-end CLI UI + command framework
...
- 🆕 automatic command creation (`bun dler dler-prompt --init cmd1 cmd2`)
- 🐦‍🔥 automatic creation of `src/app/cmds.ts` file (`bun dler dler-prompt`)
...
![dler-prompt Example CLI Screenshot](./example/example.png)
...
To help you migrate from the different CLI frameworks, `@reliverse/dler-prompt` has some aliases for the most popular prompts.
...
bun dler dler-prompt --init cmd1 cmd2 # creates `src/app/cmd1/cmd.ts` and `src/app/cmd2/cmd.ts` files
bun dler dler-prompt # creates `src/app/cmds.ts` file
```
...
import { defineCommand, createCli } from "@reliverse/dler-prompt";

const main = defineCommand({
  meta: {
    name: "dler-prompt",
    version: "1.0.0",
    description: "dler-prompt Launcher Playground CLI",
  },
  onCmdInit() {
...
```ts
import { defineArgs, defineCommand } from "@reliverse/dler-prompt";
...
- Use `bun dler dler-prompt --init cmd1 cmd2` to init commands for dler-prompt launcher's automatically
...
```ts
import { createCli, defineCommand } from "@reliverse/dler-prompt";
...
```bash
bun add -D @reliverse/dler
bun dler dler-prompt --init my-cmd-1 # or: dler dler-prompt --init my-cmd-1 my-cmd-2 --main src/mod.ts
...
const main = defineCommand({
  meta: {
    name: "mycli",
    version: "1.0.0",
    description: "CLI powered by dler-prompt",
  },
  args: {
    name: {
      type: "string",
...
const mainCommand = defineCommand({
  ...
  async run({ args, raw }) {
    ...
    relinka("log", "\nHelp: `dler-prompt --help`, `dler-prompt cmdName --help`");
...
Finally, a full-featured CLI launcher without the ceremony. `@reliverse/dler-prompt`'s so called "launcher" is a uniquely powerful and ergonomic CLI toolkit—one that helps you build delightful developer experiences with less code and more confidence. The launcher supports both programmatically defined commands and file-based routing, so you can structure your CLI however you like. It automatically detects and loads commands from your filesystem and provides robust usage and error handling out-of-the-box. The launcher is more than just a command runner—it's a robust, developer-friendly engine with several advanced features and thoughtful design choices:
...
import { relinka } from "@reliverse/relinka";
import { defineArgs, defineCommand, runCmd } from "@reliverse/dler-prompt";
...
```ts
import { runCmd } from "@reliverse/dler-prompt";
...
```ts
import { loadCommand } from "@reliverse/dler-prompt";
...
```ts
import { runCmdWithSubcommands } from "@reliverse/dler-prompt";
...
Below is a demonstration of how to define and use all supported argument types in dler-prompt: positional, boolean, string, number, and array. This includes example CLI invocations and the resulting parsed output.
...
The typed commands system provides TypeScript intellisense and type safety for dler-prompt launcher usage while maintaining dynamic code execution.
...
2. Run the generator: `dler dler-prompt --overwrite`
...
- [ ] migrate to `dler libs` in the future (all main components will be published as separate packages; `@reliverse/dler-prompt` will be a wrapper for all of them)
