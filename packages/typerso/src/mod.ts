...
export function transformExportsForBuild(
  ...
  if (Array.isArray(exports)) {
    ...
    // Each element in the array is a valid export value, so transformExportsForBuild
    // will return a valid value (never undefined for array elements)
    return exports.map((item) => {
      const result = transformExportsForBuild(item);
      // Array elements are always valid exports, so result should never be undefined
      // but TypeScript doesn't know this, so we assert the type
      return result as Exclude<PackageJson["exports"], undefined>;
  ...
  if (typeof exports === "object" && exports !== null) {
    ...
    return updated as PackageJson["exports"];
  }
