...
import os from "os";
import { cliDomainDocs, homeDir, UNKNOWN_VALUE } from "~/impl/config/constants";
import type { ReliverseConfig } from "~/impl/schema/mod";
import type { Behavior, ProjectConfigReturn } from "~/impl/types/mod";
import { experimental } from "~/impl/utils/badgeNotifiers";
import { setupI18nFiles } from "~/impl/utils/downloading/downloadI18nFiles";
import { isVSCodeInstalled } from "~/impl/utils/handlers/isAppInstalled";
import { promptPackageJsonScripts } from "~/impl/utils/handlers/promptPackageJsonScripts";
...
export async function setupI18nSupport(
	...
	const i18nFolderExists =
		(await fs.pathExists(path.join(projectPath, "src/impl/[locale]"))) ||
