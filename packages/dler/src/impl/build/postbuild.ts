...
export async function dlerPostBuild(
	...
	if (
		(await directoryExists(distJsrPath)) &&
		(await directoryExists(distJsrBinPath))
	) {
		await compareFileStructures(
			path.join(PROJECT_ROOT, config.commonEntrySrcDir),
			distJsrPath,
			isDev,
		);
	}
...
export async function wrapper_CopyNonBuildFiles(
	...
	if (
		...
		for (const [_, libConfig] of Object.entries(config.libsList)) {
			...
			if (
				libConfig.libPubRegistry === "npm" ||
				libConfig.libPubRegistry === "npm-jsr"
			) {
				await copyNonBuildFiles(
					srcPath,
...
async function compareFileStructures(
	...
	try {
		...
		if ((onlyInSrc.length > 0 || onlyInDist.length > 0) && isDev) {
			relinka(
				"warn",
				"File structure differences detected between src and dist-jsr/bin:",
			);
...
const createPostBuildToolRunner = (): Record<
	PostBuildToolName,
	PostBuildTool
> => ({
	"dler-check": {
