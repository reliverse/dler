...
export type DirConfig = Record<string, DirOptions>;

export type ExtMap = Record<string, [string, string, string]>;
...
export const DEFAULT_CONFIG: ConfigRemdn = {
  output: "table.html",
  dirs: {
    src: {},
    "dist-npm/bin": {},
...
export const expandDistLibs = async (dirs: DirConfig): Promise<DirConfig> => {
...
export const readConfig = async (path?: string): Promise<ConfigRemdn> => {
  if (!path) {
...
export const validateFilters = ({ includeExt, excludeExt }: DirOptions) => {
...
export const shouldInclude = (file: string, opts: DirOptions): boolean => {
...
export const getExpectedFilenames = (
...
export const buildTableHeader = (paths: string[]): string[] => {
  return [
...
export function mapDistLibsFolderToLibs(
  folder: string,
  colPath: string,
  allPaths: string[],
): string {
...
export const isDistLibsPath = (p: string) => p.includes("dist-libs");
...
export const ensureOutputPath = (path: string): string => {
...
export const ensureConfigPath = async (path: string): Promise<string> => {
...
export const createDefaultConfig = async (path: string): Promise<void> => {
  const ext = path.split(".").pop()?.toLowerCase();
  const isTs = ext === "ts";
