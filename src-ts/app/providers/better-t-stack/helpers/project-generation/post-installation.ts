...
import type { Database, ORM, ProjectConfig, Runtime } from "~/app/providers/better-t-stack/types";

import { getPackageExecutionCommand } from "~/app/providers/better-t-stack/utils/get-package-execution-command";
...
export function displayPostInstallInstructions(config: ProjectConfig & { depsInstalled: boolean }) {
  ...
  const starlightInstructions = addons?.includes("starlight")
    ? getStarlightInstructions(callCmd)
    : "";
  ...
  if (isConvex) {
    ...
    if (runtime === "workers") {
      ...
      output += `${re.cyan(`${stepCounter++}.`)} cd apps/server && bun run cf-typegen\n\n`;
    } else {
      output += "\n";
    }
  }
  ...
  if (hasWeb) {
    ...
    output += `${re.yellow("NOTE:")} You are creating a backend-only app (no frontend selected)\n`;
  }
  ...
  output += `${re.bold("Like Better-T Stack?")} Please consider giving us a star on GitHub:\n`;
...
function getLintingInstructions(callCmd?: string): string {
...
function getStarlightInstructions(callCmd?: string): string {
