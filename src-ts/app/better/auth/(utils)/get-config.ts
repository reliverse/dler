 ...
 */
const jitiOptions = async (cwd: string) => {
...
export async function getConfig({
  ...
  try {
    ...
    if (configPath) {
      ...
      if (await pathExists(configPath)) resolvedPath = configPath; // If the configPath is a file, use it as is, as it means the path wasn't relative.
