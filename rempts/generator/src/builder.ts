...
// Top-level regex to avoid performance issues
const COMMAND_NAME_SPLIT_REGEX = /[^a-zA-Z0-9]/;
...
/**
 * Build type registry code from command metadata
 * Returns the TypeScript code as a string without file header comment
 */
export function buildTypeRegistry(commands: CommandMetadata[]): string {
  ...
  for (const cmd of commands) {
    const baseName = getCommandVarName(cmd.name);
  ...
  const imports = commands
    .map((cmd) => {
      ...
      return `import ${varName} from '${escapeImportPath(cmd.importPath)}'`;
    })
    .join("\n");
  ...
  const metadataEntries = commands
    .map((cmd) => {
  ...
  return `import type { Command, CLI, GeneratedOptionMeta, CommandOptions, GeneratedCommandMeta } from '@reliverse/rempts-core'
import { createGeneratedHelpers, registerGeneratedStore } from '@reliverse/rempts-core'
...
export interface GeneratedCLI {
  ...
  getFlags<Name extends GeneratedNames>(name: Name): Record<string, unknown>
...
export const cli: GeneratedCLI = {
  ...
  getFlags: <Name extends GeneratedNames>(name: Name) => (generated.getFlags as any)(name),
...
/**
 * Build types file with header comment (for file-based generation)
 * Kept for backward compatibility
 */
export function buildTypes(commands: CommandMetadata[]): string {
  return `// This file was automatically generated by Rempts.
// You should NOT make any changes in this file as it will be overwritten.

${buildTypeRegistry(commands)}`;
}

function getCommandVarName(commandName: string): string {
  return (
    commandName
      .split(COMMAND_NAME_SPLIT_REGEX)
      .filter(Boolean)
...
function buildMetadataObject(command: CommandMetadata, indent: string): string {
  ...
  if (command.options && Object.keys(command.options).length > 0) {
    const optionLines = Object.entries(command.options).map(([name, option]) => {
      const optionMeta = [
        ...
        option.literalValue !== undefined
          ? `literalValue: ${JSON.stringify(option.literalValue)}`
          : undefined,
  ...
  if (command.commands && command.commands.length > 0) {
    ...
    lines.push(
      command.commands
        .map((sub) => `${indent}    ${buildMetadataObject(sub, `${indent}    `)}`)
        .join(",\n")
    );
    lines.push(`${indent}  ],`);
  }
...
function escapeImportPath(path: string): string {
  // Escape single quotes and backslashes for safe inclusion in import statements
